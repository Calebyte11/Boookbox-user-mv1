...
const OrderForm: React.FC<OrderFormProps> = ({ onSubmit, restaurantId }) => {
  ...
  const { uploadImage } = useImageUpload({
    onSuccess: () => {
      ...
      // The uploaded URL will be included in the booking payload
    },
  ...
  const formMethods = useForm<OrderFormValues>({
    defaultValues: {
      bookingType: "public", // Backend: 'public', 'self', 'others'
      redemptionMode: "pick-up", // Backend: 'pick-up' or 'delivery'
      ...
      recipientEmail: "",
      recipientAddress: "",
      recipientRemark: "", // Optional remark for single recipient
      numberOfRecipients: "1", // Maps to numberOfBookings
      multipleRecipients: [], // For bookedFor.contact (array)
      redemptionDate: new Date(), // Maps to validityDate.start
      // requestCustomTicketDesign: false,
      // ticketCustomizationRecipientName: "",
      // ticketCustomizationRecipientPhone: "",
      // ticketCustomizationRecipientEmail: "",
      // ticketCustomizationRecipientAddress: "",
      // ticketCustomizationRecipientRemark: "", // Optional remark for ticket customization recipient
      reason: "", // Booking reason
      publicTags: "", // For tags[]
      refundable: false, // Default to refundable
      supportsMultipleClaims: false, // Whether the ticket supports multiple claims
      autoGenerateTicket: false, // Default to false for auto-generating tickets
      // New fields matching backend payload      menuItems: [], // Array of { menuId, quantity, name, price, currency }
      restaurantId: restaurantId || "", // Required
      customImage: "", // Optional custom image for the ticket
      // image: "", // Restaurant image
      totalAmount: "", // Required for payment
  ...
  useEffect(() => {
    if (items.length > 0) {
      ...
      // console.log("Updated menuItems from cart:", cartMenuItems);
    } else {
      setValue("menuItems", []);
    }
  ...
  const showRecipientTabs =
    ...
    bookingTypeWatched !== "public"; // Hide recipient tabs for public bookings
  ...
  // const totalCartQuantity = useCartStore(selectTotalCartItems);
  const showTicketCustomization =
    redemptionModeWatched === "pick-up" ||
    redemptionModeWatched === "delivery" ||
    (redemptionModeWatched === "dine-in" && bookingTypeWatched === "others") ||
    (redemptionModeWatched === "dine-with-me" && (bookingTypeWatched === "others" || bookingTypeWatched === "date")) ||
    bookingTypeWatched === "public"; // Only show for bookings to others
  const showMultipleClaims =
    (bookingTypeWatched === "yourself" || bookingTypeWatched === "others") &&
    items.reduce((total, item) => total + item.quantity, 0) > 1; // Only show for single/others bookings with multiple items
  // const requestCustomTicketDesignWatched = watch("requestCustomTicketDesign");
  ...
  const handleBookingSubmit = async (data: any) => {
    // console.log("🚀 handleBookingSubmit called with data:", data);
    //   const errorMessage = Object.entries(errors).map(
    ...
    try {
      ...
      // Transform OrderForm data to booking API format
      ...
      if (uploadedTicketDesign?.file && !uploadedTicketDesign.url) {
        try {
          ...
          //   description: "Please wait while we upload your ticket design.",
          //   variant: "default",
          //   duration: 2000,
          // });
      ...
      if (isEditMode && editBookingId) {
        ...
        // console.log(
        //   "✅ Update booking validity date:",
        //   updatePayload.validityDate
        // );
        // console.log("Updating booking with payload:", updatePayload);
        ...
        // Prepare booking payload for later API call
        // console.log("💾 Form data menuItems:", data.menuItems);
        // console.log("🛒 Cart items:", items);
        // console.log("📅 Redemption date from form:", data.redemptionDate);
        ...
        // console.log(
        //   "✅ Calculated validity date:",
        //   bookingPayload.validityDate
        // );
        // console.log("Storing booking payload:", bookingPayload);
  ...
  }, [bookingTypeWatched, user, setValue, showTicketCustomization]);
    ...
    if (isEditMode && editBookingData && !isLoadingBooking) {
      ...
      if (booking.menuItems && Array.isArray(booking.menuItems)) {
        try {
          ...
          //   description:
          //     "There was an error loading your booking items. Please try again.",
          //   variant: "error",
          // });
        }
      ...
      >
        ...
        {/* {Object.keys(errors).length > 0 && (
          <div className="m-4 p-4 bg-red-50 border border-red-200 rounded-lg">
            <h3 className="font-medium text-red-800 mb-2">Required Field:</h3>
            ...
            <Controller
              ...
              render={({ field: tabField }) => (
                <Tabs.Root
                  ...
                  onValueChange={tabField.onChange}
                          ...
                          <div>
                            {bookingTypeWatched === "yourself" ? (
                              ...
                              <UserSearchCombobox
                                ...
                                onChange={(value) => setValue("recipientName", value)}
                                onUserSelect={(user: UserSearchResult) => {
                                  ...
                                  // Note: address is not provided in search results
                                  // Could be extended to fetch full user profile if needed
                                }}
                                ...
                                {bookingTypeWatched === "yourself" ? (
                                  ...
                                  <UserSearchCombobox
                                    ...
                                    onUserSelect={(user: UserSearchResult) => {
                                      ...
                                      // Address is not provided in search results
                                    }}
          ...
          <div className="m-4">
            ...
            <div className="">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Upload a custom design for your meal tickets (optional)
              </p>{" "}
              <div className="border rounded-lg overflow-hidden">
                ...
                <div className="p-6">
                  {!uploadedTicketDesign ? (
                    <div
                      className="relative border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-primary transition-colors cursor-pointer group"
                    ...
                    <div className="space-y-4">
                      ...
                      {/* <button
                        type="button"
                        onClick={handleBrowseFileClick}
                        className="w-full py-2 px-4 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
                      >
              ...
              {/* <FormField<OrderFormValues>
                name="ticketCustomizationRecipientPhone"
                type="tel"
                control={control}
                register={register}
                errors={errors}
              ...
              />
              <FormField<OrderFormValues>
                name="ticketCustomizationRecipientEmail"
                type="email"
                register={register}
                errors={errors}
                placeholder="Enter email address"
              ...
              <div className="flex justify-between px-2 mb-8">
                <FormField<OrderFormValues>
                  name="ticketCustomizationRecipientAddress"
                  register={register}
                  errors={errors}
                  placeholder="Enter street address or zipcode"
                  inputClassName={`md:w-[25rem] w-full px-3 py-4 outline-none !focus:ring-0 !border-b focus:border-b-primary`}
                />
              </div> */}
              
              {/* Ticket Customization Recipient Remark Field */}
              {/* <FormField<OrderFormValues>
                name="ticketCustomizationRecipientRemark"
                register={register}
                errors={errors}
                placeholder="Add a remark for recipient (optional)"
                inputClassName="w-full px-3 py-6 mt-[4rem] border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary h-[6rem]"
              /> */}
            </div>
          )}
        ...
        <div className="m-4">
          ...
          {/* {errors.reason && (
            <p className="text-red-500 text-xs mt-2">{errors.reason.message}</p>
          )} */}
        </div>
        {/* Refundable Section */}
        <div className="m-4">
          ...
          <Controller
            ...
            render={({ field }) => (
              <div className="space-y-3">
                ...
                >
                  <div className="p-2 flex flex-col gap-1 flex-1">
                    <h3 className="text-lg font-medium">Manual Generation</h3>
                    <p className="text-black/50 text-sm">
                      Tickets will be generated manually when needed
                    </p>
                  </div>
                  <Circle
                    fill={`${!field.value ? "#ff7a00" : "white"}`}
        ...
        {/* Submit button - can be part of this component or passed from parent */}
