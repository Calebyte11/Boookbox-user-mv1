...
const Recommended: React.FC<RecommendedType> = ({ count }) => {
  ...
  const transformedGroceries = useMemo(() => {
    ...
    return groceriesData.map((grocery: any) => ({
      id: grocery.groceryId || grocery._id || grocery.businessId || grocery.id || "",
                ...
                className={`
                  ...
                  ${
                    selectedCategory === category.id
                      ? "bg-white-500 text-gray-600 border-1 border-gray-400 rounded-t-sm rounded-b-sm"
            ...
            <Link
              to={
                item.category === "restaurant" 
                  ...
                  : `/${item.category}/${item.id || item.title}?category=${selectedCategory}`
              }
              key={item.id || index}
