...
import VegetableMarketCard from "@/components/VegetableMarketCard";
import { useNavigate } from "react-router-dom";
import { useGetAllVegetableMarkets, useGetRecommendedVegetableMarkets } from "@/hooks/useVegetableMarketQueries";
import { useLocationStore } from "@/store/locationStore";
import { formatCurrency } from "@/utils/formatCurrency";

const ViewAllVegetableMarket = () => {
  ...
  } = useGetRecommendedVegetableMarkets(
    coords?.lat || 0,
    coords?.lng || 0,
    "vegetable-market"
  );
  ...
  const markets = recommendedData && recommendedData.length > 0 ? recommendedData : allMarketsData || [];
  ...
  const filteredMarkets = useMemo(() => {
    ...
    if (selectedCity) {
      filtered = filtered.filter(
        (market) => market.city?.toLowerCase() === selectedCity.toLowerCase()
      );
    }
    ...
    if (filterBy === "recent") {
      ...
      filtered = filtered.filter((market) => market.isActive === true);
    } else if (filterBy === "inactive") {
      filtered = filtered.filter((market) => market.isActive === false);
    }

    // Apply price range filter if price data is available
    filtered = filtered.filter((market) => {
      if (market.priceRange) {
        const price = parseInt(market.priceRange.toString());
  ...
  }, [markets, searchQuery, selectedCity, filterBy, priceRange]);
      ...
      <div className="sticky top-0 z-40 bg-white shadow-sm border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h1 className="text-2xl font-bold text-slate-900">Vegetable Markets</h1>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <DropdownMenu.Root>
            ...
            >
              {["all", "recent", "active", "inactive", "city"].map((option) => (
                ...
                >
                  {option === "all" ? "All Markets" : option.charAt(0).toUpperCase() + option.slice(1)}
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Root>
        ...
        <div className="mb-6">
          <p className="text-slate-600">
            Showing <span className="font-semibold text-slate-900">{filteredMarkets.length}</span> vegetable{" "}
        ...
        {errorAll && !isLoadingAll && (
          <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-900">Error loading markets</h3>
              <p className="text-red-700 text-sm">Failed to load vegetable markets. Please try again.</p>
            </div>
          </div>
        )}
        ...
        {isRecommendedError && !isLoadingRecommended && (
          <div className="mb-6 p-4 rounded-lg bg-yellow-50 border border-yellow-200 flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-yellow-900">Showing all markets</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredMarkets.map((market) => (
              <VegetableMarketCard
                key={market._id}
                market={market}
                onClick={() => navigate(`/vegetable-market/${market._id}`)}
              />
            ))}
          </div>
        ) : (
...
export default ViewAllVegetableMarket;
