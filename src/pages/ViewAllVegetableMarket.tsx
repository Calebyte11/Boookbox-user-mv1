...
import { useAllVegetableMarketsQuery, useRecommendedVegetableMarketsQuery } from "@/hooks/useVegetableMarketQueries";
...
const ViewAllVegetableMarket = () => {
  ...
  // Fetch recommended vegetable markets when coordinates are available
  const {
    data: recommendedData,
    isLoading: isLoadingRecommended,
    isError: isRecommendedError,
  } = useRecommendedVegetableMarketsQuery(
    coords?.lat,
    coords?.lng,
    "vegetable-market",
    undefined,
  ...
  // Transform vegetable market data to proper format for display
  const items = useMemo(() => {
    if (!data) return [];
    ...
    return itemList.map((item: any) => {
      const priceRange = item.priceRange ?? 500;
      const paymentCurrency = item.paymentCurrency || "NGN";
      const rating = item.rating ?? 0;
      const id = item._id || item.vegetableMarketId || item.businessId || item.id || "";

      return {
        id,
        title: item.name || "Vegetable Market",
        image: item.image || item.profileImage || "",
        rating: rating,
        paymentCurrency,
        price: formatCurrency(priceRange, paymentCurrency),
        city: item.city || "",
        state: item.state || "",
  ...
  // Filter and sort vegetable markets based on user selections
      ...
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Vegetable Market...</p>
          </div>
        </div>
      </section>
    );
  ...
  if (showError) {
    return (
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            <p className="text-red-600 mb-4">Failed to load vegetable markets</p>
            <Button
    ...
    <section className="p-4">
      ...
      <div className="mb-4">
        <form className="relative" onSubmit={(e) => e.preventDefault()}>
          <input
            type="text"
            placeholder="Search vegetable markets..."
            value={searchQuery}
      ...
      {filteredAndSortedItems.length > 0 ? (
        ...
        <div className="flex items-center justify-center min-h-[30vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-2">No vegetable markets found</p>
            <p className="text-sm text-gray-500">
              {searchQuery
                ? "Try adjusting your search terms"
                : "No vegetable markets available at the moment"}
            </p>
          </div>
        </div>
      )}
