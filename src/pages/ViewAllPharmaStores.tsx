...
const ViewAllPharmaStores = () => {
  ...
  // Fetch recommended pharma stores when coordinates are available
  const {
  ...
  // Transform pharma store data to proper format for display
  // Extract unique cities for city filter
  // Transform pharma store data into a uniform array for UI
  ...
  const items = useMemo(() => {
    ...
    return itemList.map((item: any) => {
      ...
      const id = item.pharmaStoreId || item._id || item.businessId || item.id || "";

      return {
        id,
        title: item.name || "Pharma Stores",
  ...
  // Filter and sort pharma stores based on user selections
      ...
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Pharma Stores...</p>
          </div>
        </div>
      </section>
    );
  ...
  if (showError) {
    return (
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            <p className="text-red-600 mb-4">Failed to load pharma stores</p>
            <Button
    ...
    <section className="p-4">
      ...
      <div className="mb-4">
        <form className="relative" onSubmit={(e) => e.preventDefault()}>
          <input
            type="text"
            placeholder="Search pharma stores..."
            value={searchQuery}
      ...
      {filteredAndSortedItems.length > 0 ? (
        ...
        <div className="flex items-center justify-center min-h-[30vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-2">No pharma stores found</p>
            <p className="text-sm text-gray-500">
              {searchQuery
                ? "Try adjusting your search terms"
                : "No pharma stores available at the moment"}
            </p>
          </div>
        </div>
      )}
