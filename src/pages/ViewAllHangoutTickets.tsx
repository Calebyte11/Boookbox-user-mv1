...
const ViewAllHangoutTickets = () => {
  ...
  // Fetch recommended hangout tickets when coordinates are available
  const {
  ...
  // Transform hangout ticket data to proper format for display
  // Extract unique cities for city filter
  // Transform hangout ticket data into a uniform array for UI
  ...
  const items = useMemo(() => {
    ...
    return itemList.map((item: any) => {
      ...
      const id = item.hangoutTicketId || item._id || item.businessId || item.id || "";

      return {
        id,
        title: item.name || "Hangout Tickets",
  ...
  // Filter and sort hangout tickets
      ...
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Hangout Tickets...</p>
          </div>
        </div>
      </section>
    );
  ...
  if (showError) {
    return (
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            <p className="text-red-600 mb-4">Failed to load hangout tickets</p>
            <Button
    ...
    <section className="p-4">
      ...
      <div className="mb-4">
        <form className="relative" onSubmit={(e) => e.preventDefault()}>
          <input
            type="text"
            placeholder="Search hangout tickets..."
            value={searchQuery}
      ...
      {filteredAndSortedItems.length > 0 ? (
        ...
        <div className="flex items-center justify-center min-h-[30vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-2">No hangout tickets found</p>
            <p className="text-sm text-gray-500">
              {searchQuery
                ? "Try adjusting your search terms"
                : "No hangout tickets available at the moment"}
            </p>
          </div>
        </div>
      )}
