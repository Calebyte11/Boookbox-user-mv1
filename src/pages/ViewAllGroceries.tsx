...
const ViewAllGroceries = () => {
  ...
  // pagination state for recommended list
  // const [page] = useState(1);
  // const [limit] = useState(10);
  // Fetch all restaurants (fallback)
  const {
    data: allGroceriesData,
  ...
  // Fetch recommended restaurants when coordinates are available
  const {
  ...
  const groceriesData = useMemo(() => {
    if (Array.isArray(recommendedData) && recommendedData.length > 0) {
      return recommendedData as any;
    }
    return allGroceriesData as any;
  }, [recommendedData, allGroceriesData]);
  // Transform restaurant data to proper format for display
  // Extract unique cities for city filter
  // Transform restaurant data into a uniform array for UI

  const groceries = useMemo(() => {
    if (!groceriesData) return [];

    // Handle different response structures
    let groceriesList: any[] = [];
    if (Array.isArray(groceriesData)) {
      groceriesList = groceriesData;
    } else if (
      (groceriesData as any).data &&
      Array.isArray((groceriesData as any).data)
    ) {
      groceriesList = (groceriesData as any).data;
    } else if (
      (groceriesData as any).groceries &&
      Array.isArray((groceriesData as any).groceries)
    ) {
      groceriesList = (groceriesData as any).groceries;
    } else {
      return [];
    }

    // Transform to display format
    return groceriesList.map((groceries: any) => {
      // Ensure correct mapping for minPrice, maxPrice, averageRating
      const minPrice = groceries.minPrice ?? 1000;
      const maxPrice = groceries.maxPrice ?? 20000;
      const paymentCurrency = groceries.paymentCurrency || "NGN";
      const averageRating = groceries.averageRating ?? groceries.rating ?? 0;
      
      // Try multiple fields for ID - API may use different names
      const id = groceries.restaurantId || groceries._id || groceries.businessId || groceries.id || "";
      
      if (!id) {
        console.warn("Grocery store missing ID field. Data:", {
          name: groceries.name,
          keys: Object.keys(groceries),
          data: groceries
        });
      }
      ...
      return {
        ...
        maxPrice,
        city: groceries.city || "",
        state: groceries.state || "",
        status: groceries.isActive ? "active" : "inactive",
        createdAt: groceries.createdAt,
        updatedAt: groceries.updatedAt,
        menuId: groceries.menuId || "",
  ...
  const cityList = useMemo(() => {
    const cities = groceries.map((r: any) => r.city?.trim()).filter(Boolean);
  ...
  const filteredAndSortedGroceries = useMemo(() => {
    ...
    // City filter
    if (filterBy === "city") {
      let cityFiltered = groceries;
      if (citySearch.trim()) {
        const cityQuery = citySearch.toLowerCase().trim();
        // Filter city list for dropdown
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (groceries: any) =>
          groceries.title.toLowerCase().includes(query) ||
          groceries.price.toLowerCase().includes(query)
      );
    }
    ...
    if (filterBy === "active") {
      filtered = filtered.filter(
        (groceries: any) => groceries.status === "active"
      );
    } else if (filterBy === "inactive") {
      filtered = filtered.filter(
        (groceries: any) => groceries.status === "inactive"
      );
    }

    // Sort groceries - recent first
    ...
    return filtered;
  }, [groceries, searchQuery, filterBy, citySearch, selectedCity]);

  const handleClearSearch = () => {
    setSearchQuery("");
  };
  const isInitialLoading =
    !groceries.length && (isLoadingAll || isLoadingRecommended);
  if (isInitialLoading) {
    return (
  ...
  const showError = !groceries.length && (errorAll || isRecommendedError);
  if (showError) {
    return (
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
  ...
  return (
    <section className="p-4">
      ...
      <div className="flex items-center mb-4 md:block">
        ...
        <h1 className="text-2xl font-semibold text-center flex-1 md:text-left">
          ...
          <span className="bg-primary/10 p-1.5 px-3 rounded-lg">
            {filteredAndSortedGroceries.length}
          </span>
        </h1>
      </div>
      {/* Search Bar */}
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        {[
          { key: "all", label: "All", count: groceries.length },
          { key: "recent", label: "Recent", count: groceries.length },
          {
            key: "active",
            label: "Active",
            count: groceries.filter(
      ...
      {searchQuery && (
        <div className="mb-4 p-3 bg-[#E3F5FF] rounded-lg">
          <p className="text-sm text-gray-700">
            Showing {filteredAndSortedGroceries.length} result(s) for "
            {searchQuery}"
          </p>
        </div>
      )}{" "}
      ...
      {filteredAndSortedGroceries.length > 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredAndSortedGroceries.map(
            (groceries: any, index: number) => (
              <RestaurantCard
                key={index}
                id={groceries.id || groceries.title}
                title={groceries.title}
                image={groceries.image}
                rating={groceries.rating || 0}
                price={groceries.price}
                status={groceries.status as "active" | "inactive"}
                city={groceries.city}
                state={groceries.state}
                category="groceries"
              />
...
export default ViewAllGroceries;
