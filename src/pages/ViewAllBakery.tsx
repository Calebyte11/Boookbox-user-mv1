...
//   const { data: allData, isLoading: isLoadingAll, error: errorAll } = useAllBakeryQuery();
...
//   const { data: recommendedData, isLoading: isLoadingRecommended } = useRecommendedBakeryQuery(
//     coords?.lat ?? 0,
//     coords?.lng ?? 0,
//   );
...
//       const id = item.bakeryId || item._id || item.businessId || item.id || "";

//       return {
//         id,
//         title: item.name || "Bakery",
...
//             <input type="text" placeholder="Search bakeries..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" />
...
//         {isLoading && (<div className="text-center py-12"><div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div><p className="mt-4 text-gray-600">Loading bakeries...</p></div>)}
//         {error && (<div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3"><AlertCircle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" /><div><h3 className="font-semibold text-red-900">Error loading bakeries</h3><p className="text-sm text-red-700 mt-1">{error instanceof Error ? error.message : "Please try again later"}</p></div></div>)}
//         {!isLoading && filteredAndSortedItems.length === 0 && (<div className="text-center py-12"><p className="text-gray-600 text-lg">No bakeries found</p></div>)}
//         {!isLoading && filteredAndSortedItems.length > 0 && (<><p className="text-gray-600 mb-6">Showing {filteredAndSortedItems.length} bakery{filteredAndSortedItems.length !== 1 ? "s" : ""}</p><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">{filteredAndSortedItems.map((item: any) => (<div key={item.id} onClick={() => handleNavigateToDetail(item.id)}><RestaurantCard id={item.id} title={item.title} image={item.image} rating={item.rating} price={item.price} city={item.city} /></div>))}</div></>) }
//       </div>
//     </div>
//   );
// };

// export default ViewAllBakery;
...
import { useAllBakeryQuery, useRecommendedBakeryQuery } from "@/hooks/useBakeryQueries";
...
const ViewAllBakery = () => {
  ...
  // Fetch recommended bakeries when coordinates are available
  const {
    data: recommendedData,
    isLoading: isLoadingRecommended,
    isError: isRecommendedError,
  } = useRecommendedBakeryQuery (
    coords?.lat,
    coords?.lng,
    "bakery",
    undefined,
  ...
  // Transform bakery data to proper format for display
  // Extract unique cities for city filter
  // Transform bakery data into a uniform array for UI
  ...
  const cityList = useMemo(() => {
    ...
    return Array.from(new Set(cities)).sort();
      ...
      // Filter and sort bakeries based on user selections
 const filteredAndSortedItems = useMemo(() => {
    if (!items) return [];
    let filtered = items;
  ...
  if (showError) {
    return (
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            <p className="text-red-600 mb-4">Failed to load bakeries</p>
            <Button
      ...
      {filteredAndSortedItems.length > 0 ? (
        ...
        <div className="flex items-center justify-center min-h-[30vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-2">No Bakery found</p>
            <p className="text-sm text-gray-500">
              {searchQuery
                ? "Try adjusting your search terms"
                : "No bakery available at the moment"}
            </p>
          </div>
        </div>
      )}
