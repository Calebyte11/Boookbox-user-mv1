...
import { useAllConfectioneriesQuery, useRecommendedConfectioneriesQuery } from "@/hooks/useConfectioneryQueries";
import { useLocationStore } from "@/store/locationStore";
import { formatCurrency } from "@/utils/formatCurrency";

const ViewAllConfectionery = () => {
  ...
  const { data: allData, isLoading: isLoadingAll, error: errorAll } = useAllConfectioneriesQuery();
  ...
  const { data: recommendedData, isLoading: isLoadingRecommended } = useRecommendedConfectioneriesQuery(
    coords?.lat,
    coords?.lng,
    "confectionery",
  );
  ...
  const items = useMemo(() => {
    ...
    return itemList.map((item: any) => {
      ...
      const id = item.confectioneryId || item._id || item.businessId || item.id || "";

      return {
        id,
        title: item.name || "Confectionery",
  ...
  const handleNavigateToDetail = (id: string) => {
    if (id) navigate(`/confectionery/${id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {isLoading && (<div className="text-center py-12"><div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div><p className="mt-4 text-gray-600">Loading confectioneries...</p></div>)}
        {error && (<div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3"><AlertCircle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" /><div><h3 className="font-semibold text-red-900">Error loading confectioneries</h3><p className="text-sm text-red-700 mt-1">{error instanceof Error ? error.message : "Please try again later"}</p></div></div>)}
        {!isLoading && filteredAndSortedItems.length === 0 && (<div className="text-center py-12"><p className="text-gray-600 text-lg">No confectioneries found</p></div>)}
        {!isLoading && filteredAndSortedItems.length > 0 && (<><p className="text-gray-600 mb-6">Showing {filteredAndSortedItems.length} confectionery{filteredAndSortedItems.length !== 1 ? "s" : ""}</p><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">{filteredAndSortedItems.map((item: any) => (<div key={item.id} onClick={() => handleNavigateToDetail(item.id)}><RestaurantCard id={item.id} title={item.title} image={item.image} rating={item.rating} price={item.price} city={item.city} /></div>))}</div></>) }
      </div>
    </div>
  );
};

export default ViewAllConfectionery;
