...
const ViewAllTransportTickets = () => {
  ...
  // Fetch recommended transport tickets when coordinates are available
  const {
  ...
  // Transform transport ticket data to proper format for display
  // Extract unique cities for city filter
  // Transform transport ticket data into a uniform array for UI
  ...
  const items = useMemo(() => {
    ...
    return itemList.map((item: any) => {
      ...
      const id = item.transportTicketId || item._id || item.businessId || item.id || "";

      return {
        id,
        title: item.name || "Transport Tickets",
  ...
  // Filter and sort transport tickets
      ...
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading Transport Tickets...</p>
          </div>
        </div>
      </section>
    );
  ...
  if (showError) {
    return (
      <section className="p-4">
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center">
            <p className="text-red-600 mb-4">Failed to load transport tickets</p>
            <Button
      ...
      {filteredAndSortedItems.length > 0 ? (
        ...
        <div className="flex items-center justify-center min-h-[30vh]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-2">No transport tickets found</p>
            <p className="text-sm text-gray-500">
              {searchQuery
                ? "Try adjusting your search terms"
                : "No transport tickets available at the moment"}
            </p>
          </div>
        </div>
      )}
