...
import { useAllGiftStoresQuery, useRecommendedGiftStoresQuery } from "@/hooks/useGiftStoreQueries";
import { useLocationStore } from "@/store/locationStore";
import { formatCurrency } from "@/utils/formatCurrency";

const ViewAllGiftStores = () => {
  ...
  const { data: allData, isLoading: isLoadingAll, error: errorAll } = useAllGiftStoresQuery();
  ...
  const { data: recommendedData, isLoading: isLoadingRecommended } = useRecommendedGiftStoresQuery(
    coords?.lat,
    coords?.lng,
    "gift-stores",
  );
  ...
  const items = useMemo(() => {
    ...
    return itemList.map((item: any) => {
      ...
      const maxPrice = item.maxPrice ?? 100000;
      const paymentCurrency = item.paymentCurrency || "NGN";
      const averageRating = item.averageRating ?? item.rating ?? 0;
      const id = item.giftStoresId || item._id || item.businessId || item.id || "";

      return {
        id,
        title: item.name || "Gift Store",
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            <input type="text" placeholder="Search gift stores..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" />
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {isLoading && (<div className="text-center py-12"><div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div><p className="mt-4 text-gray-600">Loading gift stores...</p></div>)}
        {error && (<div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3"><AlertCircle className="w-5 h-5 text-red-600 mt-0.5 flex-shrink-0" /><div><h3 className="font-semibold text-red-900">Error loading gift stores</h3><p className="text-sm text-red-700 mt-1">{error instanceof Error ? error.message : "Please try again later"}</p></div></div>)}
        {!isLoading && filteredAndSortedItems.length === 0 && (<div className="text-center py-12"><p className="text-gray-600 text-lg">No gift stores found</p></div>)}
        {!isLoading && filteredAndSortedItems.length > 0 && (<><p className="text-gray-600 mb-6">Showing {filteredAndSortedItems.length} store{filteredAndSortedItems.length !== 1 ? "s" : ""}</p><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">{filteredAndSortedItems.map((item: any) => (<div key={item.id} onClick={() => handleNavigateToDetail(item.id)}><RestaurantCard id={item.id} title={item.title} image={item.image} rating={item.rating} price={item.price} city={item.city} /></div>))}</div></>) }
      </div>
    </div>
  );
};

export default ViewAllGiftStores;
